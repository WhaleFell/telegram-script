...
def init_logging():
    ...
    # 控制台输出
    logger.add(sys.stdout, level=settings.LOGGER_LEVEL, format=format_record)

    # 日志保存路径
    logger_name_path = Path(
        os.getcwd(), settings.LOGGER_DIR, settings.LOGGER_NAME
    ).as_posix()

    if settings.LOGGER_SAVE:
        # 文件输出
        logger.add(
            sink=logger_name_path,
            enqueue=True,
